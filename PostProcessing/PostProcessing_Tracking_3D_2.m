...
indx=find(settings.By_tr>=min(settings.By_tr)*0.5,1);
   
plot(s, settings.Bx_tr, '.-', 'linewidth',1);
plot(s, settings.By_tr, '-', 'linewidth',2);
plot(s, settings.Bz_tr, '--', 'linewidth',1);

% plot([s(indx(1)),s(indx(1))],[0.5,-3.25],'k--', 'linewidth',1);
%plot([s(indx(2)),s(indx(2))],[0.5,-3.25],'k--', 'linewidth',1);
...
%%
for i=1:settings.N
    y=interp3(settings.map.x,settings.map.y,settings.map.z,...
                settings.map.By,x(:,i),y(:,:),z(:,i),'linear',0)
end